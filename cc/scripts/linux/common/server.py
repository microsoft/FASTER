...
def generateCfg(args):
    ...
    if args.no_indirection: cfg["indirection"] = "false"
    if args.strict_pending: cfg["strictPends"] = "true"
    if args.splits: cfg["numSplits"] = str(args.splits)
    if args.lock: cfg["lock"] = "true"
    ...
    # Raise the limit on the max async IOs the OS can tolerate. This helps
    # when we can't store all data in DRAM.
    p = subprocess.Popen(('echo', '4194304'), stdout=subprocess.PIPE)
    o = subprocess.check_output(['sudo', 'tee', '/proc/sys/fs/aio-max-nr'],
                                stdin=p.stdout)
    p.wait()
...
"""
    Raise the limit on the number of open file descriptors. Useful if we have
    blob store enabled as the server's DFS layer.
"""
def setUlimit():
...
def run(binary, cfg, debug=False):
    ...
    try:
        subprocess.check_call(['sudo', 'chmod', '775', cfg["logDisk"]])
        subprocess.check_call(['sudo', 'chown',
    ...
    l = [binary, "--threads", cfg["threads"], "--htSizeM", cfg["bucketM"],
         ...
         "--pinOffset", cfg["pinOffset"], "--indirection", cfg["indirection"],
         "--strictPending", cfg["strictPends"], "--numSplits", cfg["numSplits"],
         "--lock", cfg["lock"]]
    if debug: l = ['gdb', '--args'] + l

    # Redirect log messages to a file and also print it to the terminal.
    p = d + '/server' + cfg["serverId"] + '.log'
    s = subprocess.Popen(l, env=e, stderr=subprocess.PIPE, preexec_fn=setUlimit)
...
def main():
    ...
    parser.add_argument('--no-indirection', action='store_true',
                        help='If true, disables indirection records and' + \
                        ' issues IOs during migration')
    parser.add_argument('--strict-pending', action='store_true',
                        help='If true, asks clients to retry when there are' + \
                        ' too many waiting requests on a session')
    parser.add_argument('--splits', type=int, required=False,
                        help='Number of ways server\'s hash range must be' + \
                        ' split for view benchmark', metavar='num')
    parser.add_argument('--lock', action='store_true',
                        help='If true, acquires ownership lock before' + \
                        ' processing a batch of requests')
