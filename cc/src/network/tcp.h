  ...
  class BufferDescriptor {
   public:
     ...
     , basePtr(nullptr)
    {
      ...
      if (r != 0) {
        logMessage(Lvl::ERR, "Failed to allocate TCP buffer (errno %d)", r);
  ...
  class Connection {
   public:
     ...
     , transport(transport)
    {
      ...
      if (flags == -1) {
        logMessage(Lvl::ERR, "Failed to retrieve socket status (errno %d)",
                   errno);
        throw std::runtime_error("Error when trying to read socket status");
      }

      if (fcntl(fd, F_SETFL, flags | O_NONBLOCK) == -1) {
        logMessage(Lvl::ERR, "Failed to make socket non-blocking (errno %d)",
                   errno);
    ...
    void sendRpc(void* context, S cb) {
      ...
      if (!alivef) {
        logMessage(Lvl::ERR, "Trying to send RPC on a dead TCP connection");
    ...
    void receive(void* context, R cb) {
      // If the connection is not alive, then log an error message and return.
      if (!alivef) {
        logMessage(Lvl::ERR, "Trying to receive on a dead TCP connection");
