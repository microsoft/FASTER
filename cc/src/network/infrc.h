  ...
  class BufferDescriptor {
   public:
     ...
     , mr(nullptr)
    {
      ...
      if (r != 0) {
        logMessage(Lvl::ERROR, "Failed to allocate Infrc buffer (errno %d)", r);
      ...
      if (!mr) {
        logMessage(Lvl::ERROR, "Failed to register Infrc buffer with the NIC");
 ...
 public:
   ...
   public:
     ...
     , nextResp(0)
    {
      ...
      if (channel) {
        ...
        if (flags == -1) {
          logMessage(Lvl::ERROR, "Failed to retrieve channel status (errno %d)",
                     errno);
    ...
    void sendRpc(void* context, S cb) {
      ...
      if (status != Status::ALIVE) {
        logMessage(Lvl::ERROR, "Trying to send RPC on a dead Infrc connection");
   ...
   private:
    ...
    inline void pollChannel() {
      ...
      if (event->event != RDMA_CM_EVENT_DISCONNECTED) {
        logMessage(Lvl::ERROR, "Received unexpected Infrc event on channel: %s",
   ...
   public:
    ...
    Connection* connect(const std::string& ip, const std::string& port) {
      ...
      if (event->event != RDMA_CM_EVENT_ADDR_RESOLVED) {
        logMessage(Lvl::ERROR, "Received unexpected Infrc event: %s",
                   rdma_event_str(event->event));
  ...
  class Poller {
   public:
    ...
    void poll() {
      ...
      if (n < 0) {
        logMessage(Lvl::ERROR, "Failed to poll server rx completion queue");
      ...
      if (m < 0) {
        logMessage(Lvl::ERROR, "Failed to poll client rx completion queue");
  ...
  void allocSharedResources(ibv_context* ctxt) {
    ...
    bool err = !pd || !serverSrq || !clientSrq || !serverRxCq || !clientRxCq ||
               !commonTxCq;
    if (err) {
      logMessage(Lvl::ERROR, "Failed to allocate shared resources for Infrc");
  ...
  void allocQueuePair(rdma_cm_id* connId, bool server) {
    ...
    if (rdma_create_qp(connId, pd, &attr) == -1) {
      logMessage(Lvl::ERROR, "Error on trying to create Infrc qp (errno %d)",
                 errno);
