...
namespace StoreVarLenTypes
{
    ...
    public class MemoryByteSample
    {
        ...
        {
            FasterKVSettings<ReadOnlyMemory<byte>, Memory<byte>> fkvSettings = new()
            {
                IndexSize = 1L << 26,
            ...
            var store = new FasterKV<ReadOnlyMemory<byte>, Memory<byte>>(fkvSettings);
            ...
            for (byte i = 0; i < 200; i++)
            {
                ...
                {
                    if (!status.Found || (!output.Item1.Memory.Slice(0, output.Item2).Span.SequenceEqual(expectedValue.Span)))
                    {
