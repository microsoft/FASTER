...
namespace FASTER.core
{
        ...
        IFasterKV<Key, Value>
    {
        ...
        /// </returns>
        public bool TakeFullCheckpoint(out Guid token, long nextVersion = -1) 
            => TakeFullCheckpoint(out token, this.FoldOverSnapshot ? CheckpointType.FoldOver : CheckpointType.Snapshot, nextVersion);
        ...
        /// operation such as growing the index). Use CompleteCheckpointAsync to wait completion.
        /// </returns>
        public bool TakeFullCheckpoint(out Guid token, CheckpointType checkpointType, long nextVersion = -1)
        {
            ...
            var result = StartStateMachine(new FullCheckpointStateMachine(backend, nextVersion));
            if (result)
        ...
        public async ValueTask<(bool success, Guid token)> TakeFullCheckpointAsync(CheckpointType checkpointType,
            CancellationToken cancellationToken = default, long nextVersion = -1)
        {
            var success = TakeFullCheckpoint(out Guid token, checkpointType, nextVersion);
        ...
        /// </param>
        /// <returns>Whether we could initiate the checkpoint. Use CompleteCheckpointAsync to wait completion.</returns>
        public bool TakeHybridLogCheckpoint(out Guid token, long nextVersion = -1)
        {
            ISynchronizationTask backend;
            if (FoldOverSnapshot)
                backend = new FoldOverCheckpointTask();
            ...
            var result = StartStateMachine(new HybridLogCheckpointStateMachine(backend, nextVersion));
            token = _hybridLogCheckpointToken;
            return result;
        }
        ...
        public async ValueTask<(bool success, Guid token)> TakeHybridLogCheckpointAsync(CheckpointType checkpointType,
            bool tryIncremental = false, CancellationToken cancellationToken = default, long nextVersion = -1)
        {
            var success = TakeHybridLogCheckpoint(out Guid token, checkpointType, tryIncremental, nextVersion);
