...
namespace FASTER.core
{
    public partial class FasterKV<Key, Value, StoreFunctions, Allocator>
    {
        ...
        {
            /// <inheritdoc/>
            public UpsertAsyncResult<Input, Output, Context> CreateResult(Status status, Output output, RecordMetadata recordMetadata) => new(status, output, recordMetadata);
                                            ...
                                            FasterExecutionContext<Input, Output, Context> currentCtx, bool asyncOp, out CompletionEvent flushEvent, out Output output)
            {
                ...
                do
                {
                    ...
                    internalStatus = fasterKV.InternalUpsert(
            ...
            /// <inheritdoc/>
            public ValueTask<UpsertAsyncResult<Input, Output, Context>> DoSlowOperation(FasterKV<Key, Value, StoreFunctions, Allocator> fasterKV, IFasterSession<Key, Value, Input, Output, Context, Allocator> fasterSession,
        ...
        public struct UpsertAsyncResult<Input, TOutput, Context>
        {
            ...
            internal UpsertAsyncResult(FasterKV<Key, Value, StoreFunctions, Allocator> fasterKV, IFasterSession<Key, Value, Input, TOutput, Context, Allocator> fasterSession,
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private ValueTask<UpsertAsyncResult<Input, Output, Context>> UpsertAsync<Input, Output, Context, FasterSession>(FasterSession fasterSession,
        ...
        private static async ValueTask<UpsertAsyncResult<Input, Output, Context>> SlowUpsertAsync<Input, Output, Context>(
            FasterKV<Key, Value, StoreFunctions, Allocator> @this,
