...
namespace FASTER.core
{
    ...
    public interface IFasterContext<Key, Value, Input, Output, Context>
    {
        ...
        /// <summary>
        /// If true, then keys must use one of the <see cref="GetLockCode(ref Key)"/> overloads to obtain a code by which groups of keys will be sorted for manual locking, to avoid deadlocks.
        /// </summary>
        ...
        /// <remarks>If <see cref="NeedKeyLockCode"/> is true, this code is obtained by FASTER on method calls and is used in its locking scheme. 
        ///     In that case the app must ensure that the keys in a group are sorted by this value, to avoid deadlock.</remarks>
        public long GetLockCode(ref Key key, long keyHash);
    }
}
