...
namespace FASTER.benchmark
{
    public class FASTER_YcsbBenchmark
    {
        ...
        Input[] input_;
        readonly FasterKVClient<Key, Value> store;
            ...
            while (!done)
            {
                ...
                for (long idx = chunk_idx; idx < chunk_idx + kChunkSize && !done; ++idx)
                {
                    ...
                    switch (op)
                    {
                        case Op.Upsert:
                            {
                                session.Upsert(ref txn_keys_[idx], ref value);
                                ++writes_done;
                                break;
                            }
                        case Op.Read:
                            {
                                session.Read(ref txn_keys_[idx], ref input, ref output);
                                ++reads_done;
                                break;
                            }
                        case Op.ReadModifyWrite:
                            {
                                session.RMW(ref txn_keys_[idx], ref input_[idx & 0x7]);
                                ++writes_done;
                                break;
                            }
                        default:
        ...
        private void SetupYcsb(int thread_idx)
        {
                ...
                chunk_idx = Interlocked.Add(ref idx_, kChunkSize) - kChunkSize)
            {
                for (long idx = chunk_idx; idx < chunk_idx + kChunkSize; ++idx)
                {
                    ...
                    session.Upsert(ref init_keys_[idx], ref value);
                }
                cnt++;
                if (cnt % 12 == 0)
                    session.CompletePending(true);
