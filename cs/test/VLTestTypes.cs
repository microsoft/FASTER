...
namespace FASTER.test
{
    ...
    public class VLFunctions : FunctionsBase<Key, VLValue, Input, int[], Empty>
    {
        public override void RMWCompletionCallback(ref Key key, ref Input input, ref int[] output, Empty ctx, Status status, RecordMetadata recordMetadata)
        {
            ...
            Assert.IsTrue(status.IsCopyRecord);
        }
        ...
        // Upsert functions
        public override void SingleWriter(ref Key key, ref Input input, ref VLValue src, ref VLValue dst, ref int[] output, ref RecordInfo recordInfo, long address, WriteReason reason)
        {
            src.CopyTo(ref dst);
        }
    ...
    public class VLFunctions2 : FunctionsBase<VLValue, VLValue, Input, int[], Empty>
    {
        ...
        // Upsert functions
        public override void SingleWriter(ref VLValue key, ref Input input, ref VLValue src, ref VLValue dst, ref int[] output, ref RecordInfo recordInfo, long address, WriteReason reason)
        {
            src.CopyTo(ref dst);
        }
