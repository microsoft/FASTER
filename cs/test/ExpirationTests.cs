...
namespace FASTER.test.Expiration
{
    ...
    internal class ExpirationTests
    {
        ...
        public class ExpirationFunctions : FunctionsBase<int, VLValue, ExpirationInput, ExpirationOutput, Empty>
        {
            ...
            // Upsert functions
            public override void SingleWriter(WriteReason reason, ref int key, ref ExpirationInput input, ref VLValue src, ref VLValue dst, ref ExpirationOutput output, ref RecordInfo recordInfo, long address)
            {
                src.CopyTo(ref dst);
            }
