...
namespace FASTER.test.LockTests
{
    ...
    internal class BasicLockTests
    {
        internal class Functions : SimpleFunctions<int, int>
        {
            ...
            public override bool ConcurrentWriter(ref int key, ref int input, ref int src, ref int dst, ref int output, ref RecordInfo recordInfo, ref UpdateInfo updateInfo) => Increment(ref dst);

            public override bool InPlaceUpdater(ref int key, ref int input, ref int value, ref int output, ref RecordInfo recordInfo, ref UpdateInfo updateInfo) => Increment(ref value);
        }
        ...
        public void FunctionsLockTest()
        {
            ...
            for (int key = 0; key < numRecords; key++)
            {
                ...
                Assert.IsFalse(session.Upsert(key, key * valueMult).Pending);
            }
            ...
            for (int key = 0; key < numRecords; key++)
            {
                ...
                Assert.IsFalse(session.Read(key, out int value).Pending);
        ...
        void UpdateFunc(bool useRMW, int numRecords, int numIters)
        {
            for (var key = 0; key < numRecords; ++key)
            {
                ...
                {
                    if ((iter & 7) == 7)
                        Assert.IsFalse(session.Read(key).status.Pending);
