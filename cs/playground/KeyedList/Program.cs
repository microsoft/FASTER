...
namespace KeyedList
{
    ...
    {
        static FasterKV<Key, CacheValue, CacheInput, CacheOutput, CacheContext, CacheFunctions> faster;
        static int numOps = 0;
        ...
        static void Main()
        {
            ...
            var serializerSettings = new SerializerSettings<Key, CacheValue> { keySerializer = () => new CacheKeySerializer(), valueSerializer = () => new CacheValueSerializer() };

            faster = new FasterKV<Key, CacheValue, CacheInput, CacheOutput, CacheContext, CacheFunctions>
        ...
        static async Task AsyncOperator(int id)
        {
            ...
            if (!batched)
            {
                ...
                while (true)
                {
                    try
                    {
                        await session.UpsertAsync(new Key(rand.Next()), new CacheValue(rand.Next()), true);
            ...
            else
            {
                ...
                while (true)
                {
                    await session.UpsertAsync(new Key(rand.Next()), new CacheValue(rand.Next()));
                    if (count++ % 100 == 0)
                    {
